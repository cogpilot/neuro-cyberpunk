...
class NeuroQuickhackQueueData : public IScriptable
{
public:
    ent::EntityID m_targetEntityId{};
...
mod::NeuroChoiceContext mod::NeuroChoiceContext::FromGameData(
    const game::interactions::vis::ListChoiceData& aChoiceData, CString& aHubTitle, int aId, bool aIsTimed,
    float aChoiceTimer)
{
...
void mod::NeuroSystem::DispatchNeuroAction(const neurosdk_message_action_t& aAction, const JobGroup& aJobGroup)
{
    ...
    JobQueue(aJobGroup).Dispatch(
        [this, response]()
        {
            ...
            // Handle action
            // TODO: refactor all JSON using actions to use REDengine JSON parser, current way is ugly
            ...
            case CNAME_HASH("drive_to_waypoint"):
            {
                ...
                if (json.destType == "id")
                {
                    ...
                    if (std::holds_alternative<std::string>(json.target))
                    {
                        ...
                        if (std::from_chars(targetStr.c_str(), targetStr.c_str() + targetStr.size(), mappinId.value)
                                .ec != std::errc())
                        {
...
void mod::NeuroSystem::TickQuickhackCache(FrameInfo& aFrameInfo, JobQueue& aJobQueue)
{
    ...
    if (!m_quickhackCacheUpdateInProgress)
    {
        ...
        Context::Spew("{} ms to gather {} quickhackable targets for cache update.", startTime.TimePassedMs().count(),
                      m_quickhackCacheProcessedObjects.size());
...
void mod::NeuroSystem::TickFuzzer(JobQueue& aQueue)
{
    static constexpr const char* NoParameterActionNames[] = {
        "OnQueryMoney",     "OnQueryTrackedQuest", "OnQueryAllQuests",       "OnQueryPlayerInfo",
        "OnQueryInventory", "OnQueryWaypoints",    "OnQueryQuickhackTargets"};
...
void mod::NeuroSystem::OnSMSMessageDataProvided(Handle<Impl::NeuroPhoneMessageDto>& aMessageInfo)
{
    ...
    auto fromGame = Impl::JSON::NeuroSMSJson::FromGame(aMessageInfo);

    auto jsonResult = glz::write_json(fromGame);
    ...
    msg->m_state = jsonResult.value().c_str();
