...
constexpr char JsonSchema[] =
    R"({ "additionalProperties": false, "type": "object", "properties": { "destType": { "description": "The type of the selected destination.", "enum": ["id", "district", "tracked"] }, "target": { "description": "The selected destination. If destType == 'district', has to be one of the districts within query_waypoints. If destType == 'id', has to be one of the IDs returned by query_waypoints. If destType == 'tracked', can be left empty.", "type": "string" } }, "required": ["destType", "target"] })";
...
constexpr char JsonSchema[] =
    R"({ "additionalProperties": false, "type": "object", "properties": { "id": { "description": "The ID of the selected dialogue option from the provided options.", "type": "integer" } }, "required": ["id"] })";
...
Choices may affect the story.)";
constexpr char JsonSchema[] =
    R"({ "additionalProperties": false, "type": "object", "properties": { "id": { "description": "The ID of the selected SMS message option from the provided options.", "type": "integer" } }, "required": ["id"] })";
...
constexpr char Desc[] =
    R"(Run a quickhack/group of quickhacks on a target. Quickhacks and targets can be queried with query_quickhackable_targets. Note that the entity ID must be exact. This should be used OFTEN when you are in combat/in a hostile area.)";
constexpr char JsonSchema[] =
    R"({ "additionalProperties": false, "type": "object", "title": "SelectQuickhack", "properties": { "entityId": { "description": "The entity ID of the quickhack target. This MUST be exact.", "type": "integer" }, "ids": { "description": "The IDs of the selected quickhack from the provided options. This has to adhere to the maximum queue length.", "type": "array", "items": { "type": "number" }  } }, "required": ["entityId", "ids"] })";
